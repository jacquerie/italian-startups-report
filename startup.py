...
def main():
    ...
    print sheet[(sheet[REVENUE_CLASS].isin(CLASSES)) & (sheet[EMPLOYEE_CLASS].isin(CLASSES))].T.apply(lambda el: el[REVENUE_CLASS] + el[EMPLOYEE_CLASS]).value_counts()
    print

    # Startups whose revenue class is smaller than their employee class.
    print sheet.loc[(sheet[REVENUE_CLASS].isin(CLASSES)) & (sheet[EMPLOYEE_CLASS].isin(CLASSES)) & (sheet[REVENUE_CLASS] < sheet[EMPLOYEE_CLASS]), BUSINESS_NAME]
    print

    # Startups whose revenue class is much bigger than their employee class.
    print sheet.loc[sheet.index[sheet[(sheet[REVENUE_CLASS].isin(CLASSES)) & (sheet[EMPLOYEE_CLASS].isin(CLASSES))].T.apply(lambda el: ord(el[REVENUE_CLASS]) > ord(el[EMPLOYEE_CLASS]) + 1).T], BUSINESS_NAME]
    print
